...
#include "alloc_sthread_next_fit_part_none.hpp"

alloc_sthread_next_fit_part_none_impl::alloc_sthread_next_fit_part_none_impl( size_t block_size ){
    _p = malloc( block_size );
    _size = block_size;
    _alloc_info.push_back( std::pair<size_t,size_t>(0,_size) );
}

alloc_sthread_next_fit_part_none_impl::alloc_sthread_next_fit_part_none_impl( void * p, size_t size ){
    _p = p;
    _size = size;
}
alloc_sthread_next_fit_part_none_impl::~alloc_sthread_next_fit_part_none_impl(){
    clear_internal();
}
bool alloc_sthread_next_fit_part_none_impl::allocating( void ** p, size_t size, bool zeroed ){
    if( size > _size ){
...
bool alloc_sthread_next_fit_part_none_impl::freeing( void * p ){
...
size_t alloc_sthread_next_fit_part_none_impl::stat_free_size_total(){
    size_t total = 0;
...
size_t alloc_sthread_next_fit_part_none_impl::stat_free_size_largest(){
    size_t largest = 0;
    for( auto it_2 = _alloc_info.begin(), it_2_e = _alloc_info.end(); it_2!=it_2_e; ++it_2 ){
...
double alloc_sthread_next_fit_part_none_impl::stat_free_size_mean(){
    size_t n = stat_free_count_blocks();
    if( n == 0 )
	return 0;
    return (double) stat_free_size_total() / stat_free_count_blocks();
}
size_t alloc_sthread_next_fit_part_none_impl::stat_free_count_blocks(){
    size_t s = _alloc_info.size();
    return s;
}
double alloc_sthread_next_fit_part_none_impl::stat_free_fraction(){
    if( _size == 0 )
	return 1.0;
    return (double) stat_free_size_total() / _size;
}
size_t alloc_sthread_next_fit_part_none_impl::stat_lent_size_total(){
    size_t total = 0;
    for( auto it = _lent_info.begin(), it_e = _lent_info.end(); it!=it_e; ++it ){
	total += it->second;
    }
...
size_t alloc_sthread_next_fit_part_none_impl::stat_lent_count_blocks(){
    return _lent_info.size();
}
double alloc_sthread_next_fit_part_none_impl::stat_lent_size_mean(){
...
bool alloc_sthread_next_fit_part_none_impl::resize_internal( void * p, size_t size, bool zeroed ){
    if( size < _size ){
...
bool alloc_sthread_next_fit_part_none_impl::resize_internal( size_t size ){
    if( size < _size ){
	assert( 0 && "resize target block too small." );
    }
    try{
...
bool alloc_sthread_next_fit_part_none_impl::clear_internal(){
    if( _lent_info.size() > 0 ){
	//some block(s) still lent out
	return false;
    }
